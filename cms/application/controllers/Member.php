...
class Member extends MY_Controller {
	...
	public function generate_barcode($id)
	{
		...
		if(!empty($member)){
			$barcode = file_get_contents('https://api.qrserver.com/v1/create-qr-code/?size=150x150&data='.config_item('base_domain').'profile/'.$member->id.'/'.url_title($member->farm, '-', true));
			if(!empty($barcode)){
